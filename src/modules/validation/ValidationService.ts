...
export class ValidationService {
    ...
    async _callSimulateValidation (userOp: UserOperation): Promise<ValidationResult> {
      const errorResult = await Config.entryPointContract.callStatic.simulateValidation(userOp, { gasLimit: 10e6 }).catch(e => e)
    ...
    async _geth_traceCall_SimulateValidation (userOp: UserOperation): Promise<[ValidationResult, BundlerCollectorReturn]> {
      ...
      const simulateCall = Config.entryPointContract.interface.encodeFunctionData('simulateValidation', [userOp])
  
      const simulationGas = BigNumber.from(userOp.preVerificationGas).add(userOp.verificationGasLimit)
  
      const tracerResult: BundlerCollectorReturn = await this.providerService.debug_traceCall({
        from: ethers.constants.AddressZero,
        to: Config.entryPointAddr,
        data: simulateCall,
      ...
      try {
        const {
          name: errorName,
          args: errorArgs
        } = Config.entryPointContract.interface.parseError(data)
        ...
        console.log('==dump tree=', JSON.stringify(tracerResult, null, 2)
