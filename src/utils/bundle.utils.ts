import { hexConcat, hexDataLength, hexDataSlice, hexZeroPad, hexlify } from 'ethers/lib/utils'
...
export function packUserOp(userOp: UserOperation): PackedUserOperation {
  ...
  if (userOp.paymaster == null) {
    ...
    if (userOp.paymasterVerificationGasLimit == null || userOp.paymasterPostOpGasLimit == null) {
  ...
  return {
    ...
    nonce: userOp.nonce,
    initCode: userOp.factory == null ? '0x' : hexConcat([userOp.factory, userOp.factoryData ?? '']),
    callData: userOp.callData,
    accountGasLimits: packUint(userOp.verificationGasLimit, userOp.callGasLimit),
