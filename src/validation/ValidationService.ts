...
import {  RpcError, getAddr, requireCond, packUserOp } from 'utils'
...
export class ValidationService {
  ...
  public async callSimulateValidation(userOp: UserOperation): Promise<ValidationResult> {
    ...
    const errorResult = await this.entryPointContract.callStatic.simulateValidation(packUserOp(userOp), { gasLimit: 10e6 }).catch((e) => e)
  ...
  public async gethTraceCallSimulateValidation(userOp: UserOperation): Promise<[ValidationResult, BundlerCollectorReturn]> {
    ...
    const simulateCall = this.entryPointContract.interface.encodeFunctionData('simulateValidation', [packUserOp(userOp)])
