...
export type SignerService = {
    ...
    */
    ...
    signTransaction(tx: Deferrable<TransactionRequest>, signer: Wallet): Promise<string>
}

// Utility function to check if a transaction is pending
const isTransactionPending = async (provider: providers.Provider, txHash: string): Promise<boolean> => {
    try {
...
export const createSignerService = (_provider: providers.Provider):SignerService  => {
    ...
    return {
        getReadySigner: async (bundleTxs: BundleTxs): Promise<number> => {
            ...
            const pendingStatuses = await Promise.all(
                Object.values(bundleTxs).map(async ({ txHash, signerIndex }) => {
                    const isPending = await isTransactionPending(provider, txHash)
                    return [signerIndex, isPending] as [number, boolean]
                })
            );

            const availableSigner = pendingStatuses.find(([_, isPending]) => !isPending)
        ...
        signTransaction: async (tx: Deferrable<TransactionRequest>, signer: Wallet): Promise<string> => {
