...
export const createCommonEventManager = (
  ...
  return {
    getUserOperationEvent: async (userOpHash: string): Promise<ethers.Event> => {
      // TODO: eth_getLogs is throttled. must be acceptable for finding a UserOperation by hash
      const event = await entryPointContract.queryFilter(entryPointContract.filters.UserOperationEvent(userOpHash))
      return event[0]
    },
    ...
    filterLogs: (userOpEvent: ethers.Event, logs: Log[]): Log[] => {
      ...
      const beforeExecutionTopic = entryPointContract.interface.getEventTopic(foundEvent)
