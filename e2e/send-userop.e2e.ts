...
const bundlerProvider = new ethers.providers.StaticJsonRpcProvider(
...
const hdNode = ethers.utils.HDNode.fromMnemonic(
  process.env.TRANSEPTOR_MNEMONIC as string,
)
...
async function main() {
  ...
  // Estimate gas
  const gasEstimate = await bundlerProvider
  ...
  //TODO: Figure out why to estimate gas `preVerificationGas` and `verificationGasLimit` value are not working correctly when calling the `eth_estimateUserOperationGas` method
  // using harded coded values for now
  ...
  const userOpHash = await bundlerProvider
